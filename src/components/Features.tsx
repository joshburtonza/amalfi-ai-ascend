  ...
  const features = [
    {
      ...
      icon: (
        <svg className="w-8 h-8 md:w-10 md:h-10 mb-4 md:mb-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <section id="features" className="py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="max-container relative z-10 px-4 sm:px-6">
        <h2 className="text-2xl md:text-4xl font-bold mb-10 md:mb-16 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {features.map((feature, index) => (
            <div
              ...
              className="glassmorphic rounded-2xl p-6 md:p-8 border border-amalfi-emerald/10 shadow-glow-sm hover:shadow-glow-md transition-all duration-500 opacity-0 translate-y-10"
              style={{ transitionDelay: `${index * 150}ms` }}
            >
              <div className="text-gradient">{feature.icon}</div>
              <h3 className="text-xl md:text-2xl font-bold mb-3 md:mb-4 text-amalfi-white">{feature.title}</h3>
              <p className="text-sm md:text-base text-amalfi-white/80">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
